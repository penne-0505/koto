...
const runFrontmatterSchemaMarkerCase = async () => {
  const repoRoot = Deno.cwd();
  const temp = await Deno.makeTempDir({ prefix: "docs-dd-frontmatter-" });
  try {
    await ensureDir(`${temp}/_docs/intent/Core/frontmatter-marker`);
    await ensureDir(`${temp}/_docs/qa/Core/frontmatter-marker`);
...
related_prs: []`;
    await write(
      `${temp}/_docs/intent/Core/frontmatter-marker/decision.md`,
      `---
title: Intent schema marker fixture
intent_schema: 2
${common}
future_unknown_marker: enabled
---
`,
    );
    await write(
      `${temp}/_docs/qa/Core/frontmatter-marker/test-plan.md`,
      `---
title: QA schema marker fixture
qa_schema: 2
qa_status: planned
risk: Medium
${common}
...
`,
    ...
    const command = new Deno.Command(deno, {
      ...
      env: {
        DD_SCOPE_PATHS:
          "_docs/intent/Core/frontmatter-marker/decision.md:_docs/qa/Core/frontmatter-marker/test-plan.md",
      },
      stdout: "piped",
      stderr: "piped",
    });
    ...
    return output.code === 0 &&
      stderr.includes("unknown field: future_unknown_marker") &&
      !stderr.includes("unknown field: intent_schema") &&
      !stderr.includes("unknown field: qa_schema");
  } finally {
...
ok = await (async () => {
  const passed = await runFrontmatterSchemaMarkerCase();
  if (passed) {
    console.log(
      "PASS frontmatter accepts schema markers and warns on unknown fields",
    );
    return true;
  }
  console.error(
    "FAIL frontmatter schema markers or unknown-field warning contract",
  );
  return false;
})() && ok;
